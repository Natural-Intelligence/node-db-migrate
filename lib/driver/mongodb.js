...
var MongodbDriver = Base.extend({
  ...
  switchDatabase: function(options, callback) {
    ...
    if (typeof(options) === 'object') {
      if (typeof(options.database) === 'string')
        this._run('use', options.database, null, callback);
    }
    else if (typeof(options) === 'string') {

      this._run('use', options, options, callback);
    }
    else
      callback(null);
  },
  ...
  _run: function (command, collection, options, callback) {
    log.info(`run collection:${collection} options:${options}`);
    ...
    if (global.migrationCount && global.migrationCount >= 3 && options !== null && typeof(options) === 'object') {
      if (options.name) {
        const firstdir = options.name.split('/')[0];

        var reqstr = `${process.cwd()}/migrations/${firstdir}/config.json`;
        log.info(`require ${reqstr}`);
        var obj = null;
        ...
        if (obj != null && obj.database) {
          const database = obj.database;
          //console.log(`db:${obj.database}`);
          const last = connectionString.lastIndexOf('/');
          const connname = connectionString.substring(0, last + 1);
          connectionString = connname + database;
          log.info(`switcheds to ${connectionString}`);
        }
      }
    }

    log.info(`print the connection: ${connectionString}`);
...
exports.connect = function(config, callback) {
  ...
  if(config.user !== undefined && config.password !== undefined && config.authdb !== undefined) {
    mongoString += `?replicaSet=${config.replicaSetName}&authSource=${config.authdb}`;
  }
  log.info(`printing mongoString: ${mongoString}`);
  //console.log(`printing mongoString: ${mongoString}`);
